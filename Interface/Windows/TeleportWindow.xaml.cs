	...
	public partial class TeleportWindow
	{
		...
		public async Task GetCoordsFromAoB()
		{
			try
			{
				...
				AoBCoordsResults = (await _m.AoBScan("FF FF FF 7F 00 00 00 00 00 00 ?? ?? ?? 00 00 00 ?? ?? ?? ?? 00 00 00 00 00 00 00 00 00 00 00 00 ?? ?? ?? ?? 00 00 00 00 00 00 00 00 00 00 00 00 ?? ?? ?? ?? 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 80 3F 00 00 00 00 00 00 03 3C", true)).ToArray();

				selected_address.Text = AoBCoordsResults[0].ToString();
				CoordAddress = Convert.ToInt64(selected_address.Text);
		...
		private void Poke_All_Coords(object sender, RoutedEventArgs e)
		{
			...
			try
			{
				foreach (var coord in AoBCoordsResults)
				{
					CoordAddress = Convert.ToInt64(coord);
					_m.WriteMemory(((CoordAddress + 0x54).ToString("X")), "float", x_des.Text);
					_m.WriteMemory(((CoordAddress + 0x58).ToString("X")), "float", y_des.Text);
					_m.WriteMemory(((CoordAddress + 0x5C).ToString("X")), "float", z_des.Text);
				}
